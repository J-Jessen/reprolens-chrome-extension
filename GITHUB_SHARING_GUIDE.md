...
Share the official [v0.11.1-beta.1 release](https://github.com/J-Jessen/consolehawk-chrome-extension/releases/tag/v0.11.1-beta.1).

Ask each tester to:

1. download `consolehawk-v0.11.1-beta.1.zip` from **Assets**;
...
ConsoleHawk is source-available for beta evaluation and is not currently open source. The allowed evaluation use and restrictions are stated in `LICENSE.md`. Public source reduces distribution friction, but it cannot technically prevent someone from copying files they can view.

## Safe public reporting

Use the repository's **Beta bug report** and **Beta feedback** issue forms for ordinary sanitized reports.

Never put credentials, cookies, tokens, customer data, private URLs, raw traces, or unreviewed exports in a public issue. Use [private vulnerability reporting](https://github.com/J-Jessen/consolehawk-chrome-extension/security/advisories/new) for an undisclosed vulnerability, a privacy failure, or accidental sensitive-data exposure.
...
4. open a pull request against `J-Jessen/consolehawk-chrome-extension:main`;
...
> Thanks for helping test ConsoleHawk. Download the installable ZIP from the official v0.11.1-beta.1 release and begin with `START_HERE.md` inside the extracted folder:
>
> https://github.com/J-Jessen/consolehawk-chrome-extension/releases/tag/v0.11.1-beta.1
>
> Choose the file named `consolehawk-v0.11.1-beta.1.zip` under Assets, not a Source code archive. Test only on a local or staging site you are authorized to inspect. Never post credentials, customer data, private URLs, raw traces, or unreviewed exports in a public issue.
