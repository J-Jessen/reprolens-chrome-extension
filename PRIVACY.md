...
Completed traces can be retained in `chrome.storage.local`, with a maximum of 25 traces. History can be disabled or cleared from the side panel. Uninstalling the extension removes its local storage according to Chrome's extension-storage behaviour.

## Export

JSON and Markdown exports are explicit user actions. The export layer masks sensitive URL parameters, email addresses, bearer/JWT-like credentials, known secret fields, and DOM value attributes. A full JSON preview is shown before download. Pattern-based redaction cannot guarantee that every form of private data is detected, so users must review an export before sharing it.
