# ConsoleHawk public beta promotion kit

Use the official release link in every post:

https://github.com/J-Jessen/consolehawk-chrome-extension/releases/tag/v0.11.1-beta.1

Use the public beta page when the reader should understand the product before downloading:

https://j-jessen.github.io/consolehawk-chrome-extension/

Use `assets/consolehawk-social-preview.png` as the share image. Do not ask for upvotes. Ask for one concrete test and honest feedback.

## Core description

ConsoleHawk is a local-first Chrome extension that records one browser interaction or short user journey, explains the related handlers, requests, asynchronous work, navigation, and page changes, then builds a privacy-reviewed bug report and Playwright test skeleton.
...
I am looking for developers and QA engineers to test ConsoleHawk, a public-beta Chrome extension for debugging browser behaviour.

Select an element, perform one interaction, and ConsoleHawk builds a local explanation and timeline covering JavaScript handlers, requests, asynchronous work, navigation, and visible page changes. It can also turn a short journey into a privacy-reviewed bug report and Playwright test skeleton.
...
3. Trace the same failure with ConsoleHawk.
4. Tell me whether it improved your diagnosis and first debugging step.

See the product: https://j-jessen.github.io/consolehawk-chrome-extension/

Volunteer for a guided 15-minute session: https://github.com/J-Jessen/consolehawk-chrome-extension/issues/new?template=guided-beta-session.yml
...
You test ConsoleHawk, I test yours — looking for honest developer-tool feedback

### Post

I have opened the public beta of ConsoleHawk, a local-first Chrome extension that follows one browser interaction through handlers, requests, asynchronous work, and page changes, then creates a reviewed bug report and Playwright test skeleton.
...
See the product and safe demo: https://j-jessen.github.io/consolehawk-chrome-extension/
...
I have been building ConsoleHawk to test whether browser debugging evidence can be made understandable without pretending that timing proves causality.
...
Product, safe demo, and test links: https://j-jessen.github.io/consolehawk-chrome-extension/

## Show HN

### Title

Show HN: ConsoleHawk – turn browser interactions into bug reports and Playwright tests

### First comment

I built ConsoleHawk because browser bug reports often describe only the visible symptom, while the useful evidence is split across event listeners, Network, Console, source maps, and page changes.

ConsoleHawk records one user-started interaction or short journey and assembles that evidence locally. Its default explanation is deterministic and conservative: direct relationships are separated from timing-only observations. It can then generate a reviewed bug report and Playwright test skeleton without uploading a trace or storing a GitHub token.
...
Hi [name] — I have opened the public beta of ConsoleHawk, a Chrome debugging extension that turns one browser interaction into a plain-language explanation, technical trace, safe bug report, and Playwright test skeleton.
